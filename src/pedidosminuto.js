 ...
 */
	pedidos_minuto: async function(ctx, bot, enviaImagem, enviaDocumento,id_marca)  {
	
	var sql_query = query.queryPedidosMinuto(id_marca);
	  ...
	  resultSet.getRows(
	    ...
	    function (err, rows) {
	      if (err) {
			...
			var header="MINUTO" + "\;" + "QTD PEDIDOS MINUTO" + "\;" + "MARCA" + "\n"

			for (var i = 0; i < rows.length; i++) {
				eixoX[i] = moment(rows[i].MINUTO).format('h:mm a');
				eixoY[i] = rows[i].PEDIDOS;
				corpoDocumento += moment(rows[i].MINUTO).format('h:mm a') + '\;' 
			...
			if (enviaDocumento){
				var fs = require("fs");
				var file = 'qtd_pedidos_minuto.csv';
			...
			if (enviaImagem) {
			    ...
			    let layout = {  // Chart Layout
			            title: nome + ': Pedidos ' + controle_marca + ' por Minuto',   // Chart Title
			            xaxis: {
			                title: 'Minuto'    // X axis title
			            },
			            yaxis: {
			    ...
			    var fileName = 'minuto' + chatId + '.png';
